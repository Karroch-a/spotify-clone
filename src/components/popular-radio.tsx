...
import { popularRadio } from "@/data/music-data";

export function PopularRadio() {
  const { currentSong, isPlaying, playSong, toggleLikeSong, isLiked } =
    useAuth();
  const router = useRouter();

  const handlePlayRadio = (
    radio: (typeof popularRadio)[0],
    e: React.MouseEvent
  ) => {
    e.stopPropagation();
    playSong(radio.featuredSong);
  };
  ...
  const navigateToPlaylist = (radio: (typeof popularRadio)[0]) => {
    router.push(`/playlist/${radio.id}`);
  };
    ...
    <section className="space-y-4">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
        {popularRadio.map((radio) => {
          const isCurrentRadio = currentSong?.id === radio.featuredSong.id;
          const songIsLiked = isLiked(radio.featuredSong.id);

          return (
            <div
              key={radio.id}
              className="group relative min-w-[180px] bg-zinc-900/40 p-4 rounded-md hover:bg-zinc-800/60 transition-all cursor-pointer"
              onClick={() => navigateToPlaylist(radio)}
            >
              <div className="relative mb-4">
                ...
                <Button
                  ...
                  onClick={(e) => handlePlayRadio(radio, e)}
                >
                ...
                <Button
                  ...
                  onClick={(e) => handleLikeSong(e, radio.featuredSong)}
                >
                  <Heart
